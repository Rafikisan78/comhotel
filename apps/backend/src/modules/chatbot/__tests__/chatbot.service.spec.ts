...
describe('ChatbotService - Security Tests', () => {
  ...
  describe('System Manipulation Protection (OWASP LLM08)', () => {
    const systemManipulationTests = [
      ...
      { name: 'require injection', input: "require('child_process').exec('ls')" },
  ...
  describe('XSS Protection', () => {
    const xssTests = [
      ...
      { name: 'iframe injection', input: '<iframe src="http://evil.com"></iframe>' },
      { name: 'object tag', input: '<object data="http://evil.com"></object>' },
      { name: 'embed tag', input: '<embed src="http://evil.com">' },
      { name: 'data URI XSS', input: 'data: text/html,<script>alert(1)</script>' },
    ];
  ...
  describe('Rate Limiting (OWASP LLM04, MITRE AML.T0040)', () => {
    it('should allow requests within rate limit', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        const response = await service.processMessage(sessionId, 'hôtels à Paris');
  ...
  describe('Input Length Validation', () => {
    it('should block messages exceeding maximum length', async () => {
      ...
      const response = await service.processMessage('session-length', longMessage);
    ...
    it('should accept messages within length limit', async () => {
      ...
      const response = await service.processMessage('session-valid', validMessage);
  ...
  describe('Valid Hotel Search Queries', () => {
    it('should process valid city search', async () => {
      const response = await service.processMessage('session-search-1', 'Hôtels à Paris');
    ...
    it('should process valid star rating search', async () => {
      const response = await service.processMessage('session-search-2', 'Hôtel 4 étoiles');
    ...
    it('should process valid amenity search', async () => {
      const response = await service.processMessage('session-search-3', 'Hôtel avec piscine et spa');
    ...
    it('should process valid guest count search', async () => {
      const response = await service.processMessage('session-search-4', 'Chambre pour 3 personnes');
  ...
  describe('Output Validation (OWASP LLM02)', () => {
    it('should only return allowed response types', async () => {
      const allowedTypes = ['hotel_list', 'hotel_filter', 'availability_check', 'error', 'help'];
    ...
    it('should filter sensitive hotel data from response', async () => {
      const response = await service.processMessage('session-filter', 'Hôtels à Paris');
      ...
      if (response.data?.hotels && response.data.hotels.length > 0) {
        ...
        const allowedFields = ['id', 'name', 'slug', 'city', 'country', 'stars', 'short_description', 'cover_image', 'amenities', 'is_featured'];
    ...
    it('should sanitize output message', async () => {
      const response = await service.processMessage('session-sanitize', 'Hôtels');
  ...
  describe('Adversarial Input Handling (MITRE AML.T0043)', () => {
    ...
    adversarialTests.forEach(({ name, input }) => {
      it(`should handle ${name} safely`, async () => {
        const response = await service.processMessage('session-adversarial', input);
...
describe('ChatbotService - Functional Tests', () => {
  ...
  beforeEach(async () => {
    ...
    const createChainableMock = () => {
      const defaultResult = {
        data: [
          { id: '1', name: 'Hotel Paris', city: 'Paris', stars: 4, slug: 'hotel-paris' },
          { id: '2', name: 'Hotel Moroni', city: 'Moroni', stars: 5, slug: 'hotel-moroni' },
        ],
        error: null,
      };
      ...
      mock.limit = jest.fn().mockImplementation(() => Promise.resolve(mock._result));
  ...
  describe('Intent Recognition', () => {
    it('should recognize city search intent', async () => {
      ...
      const response = await service.processMessage('func-test-1', 'Je cherche un hôtel');
    ...
    it('should recognize star rating intent from message', async () => {
      const response = await service.processMessage('func-test-2', 'Hôtel 5 stars disponible');
  ...
  describe('Error Handling', () => {
    it('should handle database errors gracefully', async () => {
      ...
      const response = await service.processMessage('func-test-5', 'Hôtels disponibles');
