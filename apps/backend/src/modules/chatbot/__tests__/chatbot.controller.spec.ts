...
describe("ChatbotController", () => {
  ...
  describe("POST /chatbot/message", () => {
    ...
    it("should reject null message", async () => {
      await expect(
        controller.sendMessage(
          { message: null as any, sessionId: "session-123" },
          mockRequest,
        ),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should reject invalid sessionId format (special chars)", async () => {
      await expect(
        controller.sendMessage(
          { message: "Test", sessionId: "session<script>" },
          mockRequest,
        ),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should reject invalid sessionId format (SQL injection)", async () => {
      await expect(
        controller.sendMessage(
          { message: "Test", sessionId: "session'; DROP TABLE--" },
          mockRequest,
        ),
      ).rejects.toThrow(BadRequestException);
    });
  ...
  describe("GET /chatbot/history/:sessionId", () => {
    it("should return history for valid sessionId", () => {
      ...
      expect(mockChatbotService.getConversationHistory).toHaveBeenCalledWith(
        "valid-session-123",
      );
    });

    it("should reject invalid sessionId format", () => {
      expect(() => controller.getHistory("bad<id>")).toThrow(
        BadRequestException,
      );
    });
  ...
  describe("DELETE /chatbot/session/:sessionId", () => {
    it("should reset valid session", () => {
      ...
      expect(mockChatbotService.resetSession).toHaveBeenCalledWith(
        "valid-session-123",
      );
    });

    it("should reject invalid sessionId format", () => {
      expect(() => controller.resetSession("invalid;session")).toThrow(
        BadRequestException,
      );
    });
  });
...
describe("ChatbotController - Input Validation Edge Cases", () => {
  ...
  describe("SessionId boundary validation", () => {
    ...
    it("should reject sessionId exceeding maximum length", async () => {
      ...
      await expect(
        controller.sendMessage(
          { message: "Test", sessionId: tooLongSessionId },
          mockRequest,
        ),
      ).rejects.toThrow(BadRequestException);
    });
  ...
  describe("Type validation", () => {
    it("should reject non-string message type", async () => {
      await expect(
        controller.sendMessage(
          { message: 123 as any, sessionId: "session-123" },
          mockRequest,
        ),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should reject non-string sessionId type", async () => {
      await expect(
        controller.sendMessage(
          { message: "Test", sessionId: 123 as any },
          mockRequest,
        ),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should reject array message", async () => {
      await expect(
        controller.sendMessage(
          { message: ["test"] as any, sessionId: "session-123" },
          mockRequest,
        ),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should reject object message", async () => {
      await expect(
        controller.sendMessage(
          { message: { text: "test" } as any, sessionId: "session-123" },
          mockRequest,
        ),
      ).rejects.toThrow(BadRequestException);
    });
